...
const resultt = result.replace(/\€/g, ",sentence,end.").replace(/\:/g, ',semicolon.').replace(/\¢/g, ",comma.").replace(/\ক/g, ',chuto,b.').replace(/\খ/g, ',Boro,v.').replace(/\গ/g, ',chuto,n.').replace(/\ঘ/g, ',Boro,m.').replace(/\ঙ/g, ',phhiie.').replace(/\চ/g, ',yiiiee.').replace(/\ছ/g, ',wo.').replace(/\জ/g, ',yiiuu.').replace(/\ঝ/g, ',ass.').replace(/\ঞ/g, ',arrrerr.');
